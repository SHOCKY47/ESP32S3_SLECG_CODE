...
> **一颗芯片，两种通道，一条心跳。**  
> 基于 ESP32-S3 的单导联可穿戴心电采集固件 —— 把 ADS1291 的 500 Hz 波形，通过 **UART 有线** 或 **BLE 无线** 送到你的上位机。

```
   皮肤电极                ESP32-S3                    上位机
  ┌────────┐    SPI/DRDY   ┌──────────┐   UART0 / BLE   ┌─────────┐
  │ ADS1291│ ────────────► │ SLECG    │ ──────────────► │ 波形 UI │
  │  1ch   │   500 Hz      │ 固件     │   A5 5A 帧      │  存储   │
  └────────┘               └──────────┘                 └─────────┘
                                │
                           GPIO18 按键
                           GPIO41 绿灯 / GPIO42 蓝灯
...
| 能力 | 说明 |
|------|------|
| **硬件前端** | TI ADS1291，单通道，500 SPS，24-bit 原始采样 |
...
├── main/                      # 入口：初始化 + 启动应用层
│   ├── main.c
│   └── board_pins.h           # 板级 GPIO 定义
├── components/
│   ├── ads129x/               # ADS1291 SPI 驱动（RDATAC、寄存器、DRDY）
...
| 1 | 电池 ADC（预留，未启用） |
| 2 | ADS1291 DRDY |
| 3–5 | SPI MOSI / MISO / SCLK |
| 6–8 | SPI CS / START / PWDN |
| **18** | **用户按键**（按下拉低） |
| **41** | **绿灯** — UART 模式指示 |
| **42** | **蓝灯** — BLE 模式指示 |
